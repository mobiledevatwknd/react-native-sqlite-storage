var SQLite3JS = (function () {
  ...
  function wrapDatabase(connection) {
    ...
    that = {
      ...
      totalChanges: function() {
        return connection.totalChanges;
      },
  ...
  SQLite3JS.openAsync = function (dbPath, options) {
    /// <summary>
    ...
    var openWithKey = !!options.key && options.key.length !== 0;

    var promise = openWithKey ? SQLite3.Database.openAsync(dbPath, key) : SQLite3.Database.OpenAsync(dbpath);
    promise.then(
      connection => {
